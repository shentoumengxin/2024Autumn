...
public:
    ...
    node(long long val = 0, point* p = nullptr) : num(val), p(p), dist(LLONG_MAX), is_visit(false) {}
};
...
int main() {
    ...
    while (!heap.empty()) {
        ...
        for (int i = 0; i < current->p->neibor.size(); i++) {
            ...
            // 如果邻接点未被访问且边权更小，则更新
            if (!dist[v] && weight < dist[v]) {  // 注意：这里原代码有误，应为 dist[v] > weight
                if (weight < dist[v]) {
                    dist[v] = weight;
